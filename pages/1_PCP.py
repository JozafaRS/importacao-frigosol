...
def processar_confrigo(arquivo):
    ...
    try:
        novos_dados = up.filtrar_novos_dados(data_frame, 'pcp_confrigo')
    except Exception as e:
        progresso.empty()
    ...
    try:
        up.adicionar_registros(df_formatado, 'pcp_confrigo')
...
def processar_frigosol(arquivo):
    ...
    try:
        novos_dados = up.filtrar_novos_dados(data_frame, 'pcp_frg')
    except Exception as e:
        progresso.empty()
        st.error(f'Erro ao se conectar ao banco de dados. Error: {e}')
        return
    ...
    try:
        up.adicionar_registros(df_formatado, 'pcp_frg')
        progresso.progress(5/5, 'Finalizado!')
        progresso.empty()
        st.success(f'Base de dados atualizada com sucesso! {len(df_formatado)} registros adicionados')
    except Exception as e:
...
def page_upload():
    ...
    with col1:
        ...
        st.divider()
        arquivo_agropecuaria = st.file_uploader("**Planilha PCP Agropecuária**", ['xlsx', 'xls'])
        arquivo_confrigo = st.file_uploader('**Planilha PCP Confrigo**', ['xlsx', 'xls'])
        arquivo_frigosol = st.file_uploader('**Planilha PCP Frigosol**', ['xlsx', 'xls'])
